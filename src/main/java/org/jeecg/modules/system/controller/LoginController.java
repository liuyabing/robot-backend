...
public class LoginController {
	...
	private Result<JSONObject> userInfo(SysUser sysUser, Result<JSONObject> result) {
		...
		String rongToken = "";
		//查询该用户的融云信息返回给用户
		QueryWrapper<RongUser> queryWrapper = new QueryWrapper<RongUser>();
		queryWrapper.eq("user_id", sysUser.getId());
		try {
			RongUser rongUser = rongUserService.getOne(queryWrapper);
			if(null == rongUser) {
				//没有查找到rongUser
		        ...
		        UserModel userModel = new UserModel()
		                ...
		                .setPortrait(LelianConstants.AVATAR);
		        //注册用户，生成用户在融云的唯一身份标识 Token
		        TokenResult rongResult = User.register(userModel);
		        //写到融云用户表中
		        if(rongResult.getCode() == 200) {
		        	RongUser rongUserObj = new RongUser();
		        	rongUserObj.setUserId(userModel.getId());
		        	rongUserObj.setUserName(userModel.getName());
		        	rongUserObj.setUserType(LelianConstants.LELIAN_USER_TYPE_COMMON);
		        	rongUserObj.setToken(rongResult.getToken());
		        	rongUserObj.setPortrait(userModel.getPortrait());
		        	rongUserObj.setCreateBy(userModel.getId());
		        	rongUserObj.setCreateTime(new Date());
		        	
		        	rongUserService.save(rongUserObj);
		        }
		        rongToken = rongResult.getToken();
			} else {
				rongToken = rongUser.getToken();
			}
		}catch (Exception e) {
			e.printStackTrace();
		}
		obj.put("rongToken", rongToken);
		result.setResult(obj);
		result.success("登录成功");
		return result;
	}
	...
	@ApiOperation("刷新jwt token")
	public String refreshJwtToken(String token, String userName) {
		// 查询用户信息
		SysUser sysUser = sysUserService.getUserByName(userName);
		if (sysUser == null) {
		...
		if (oConvertUtils.isNotEmpty(cacheToken) 
				&& JwtUtil.verify(token, userName, sysUser.getPassword())) {
		...
		//如果缓存里没有该用户token或者用户token超期,重新生成token
		String newAuthorization = JwtUtil.sign(userName, sysUser.getPassword());
